 ...
 *       - Redesign functions that require a value as parameter to be returned,
 *pass by reference
 ...
 *       - No auto-layout mechanism provided, up to the user to define controls
 *position and size
 ...
 *
 *   DEPENDENCIES:
 *       raylib 4.5          Inputs reading (keyboard/mouse), shapes drawing,
...
typedef enum {
  TEXT_INNER_PADDING =
      16, // TextBox/TextBoxMulti/ValueBox/Spinner inner text padding
  TEXT_LINES_SPACING,      // TextBoxMulti lines separation
  TEXT_ALIGNMENT_VERTICAL, // TextBoxMulti vertical alignment: 0-CENTERED, 1-UP,
                           // 2-DOWN
  TEXT_MULTILINE,          // TextBox supports multiple lines
  TEXT_WRAP_MODE // TextBox wrap mode for multiline: 0-NO_WRAP, 1-CHAR_WRAP,
                 // 2-WORD_WRAP
} GuiTextBoxProperty;
...
RAYGUIAPI Rectangle GuiScrollPanel(Rectangle bounds, const char *text,
                                   Rectangle content,
...
RAYGUIAPI int
GuiToggleGroup(Rectangle bounds, const char *text,
...
RAYGUIAPI Vector2
GuiGrid(Rectangle bounds, const char *text, float spacing,
...
GuiTextInputBox(Rectangle bounds, const char *title, const char *message,
                ...
                int *secretViewActive); // Text Input Box control, ask for text,
                                        // supports secret
RAYGUIAPI Color
GuiColorPicker(Rectangle bounds, const char *text,
               Color color); // Color Picker control (multiple color controls)
RAYGUIAPI Color GuiColorPanel(Rectangle bounds, const char *text,
                              Color color); // Color Panel control
RAYGUIAPI float GuiColorBarAlpha(Rectangle bounds, const char *text,
                                 float alpha); // Color Bar Alpha control
RAYGUIAPI float GuiColorBarHue(Rectangle bounds, const char *text,
...
static unsigned int
    guiIcons[RAYGUI_ICON_MAX_ICONS * RAYGUI_ICON_DATA_ELEMENTS] = {
        ...
        0x7fe00000, 0x50286030, 0x47fe4804, 0x44224402, 0x44224422,
        0x3ff27fe2, 0x0ffe1ffa, 0x000007fe, // ICON_CUBE_FACE_BOTTOM
        ...
        0x24127fe2, 0x0c06140a, 0x000007fe, // ICON_CUBE_FACE_BACK
...
static unsigned int textBoxCursorIndex =
    0; // Cursor index, shared by all GuiTextBox*()
...
//-------------------------------------------------------------------------------
static Font LoadFontEx(const char *fileName, int fontSize, int *fontChars,
...
static void GuiDrawText(const char *text, Rectangle bounds, int alignment,
...
#define RAYGUI_GROUPBOX_LINE_THICK 1
  ...
  GuiDrawRectangle(
      RAYGUI_CLITERAL(Rectangle){bounds.x, bounds.y, RAYGUI_GROUPBOX_LINE_THICK,
                                 bounds.height},
      0, BLANK,
      Fade(GetColor(GuiGetStyle(DEFAULT, (state == STATE_DISABLED)
                                             ? BORDER_COLOR_DISABLED
                                             : LINE_COLOR)),
           guiAlpha));
  GuiDrawRectangle(
...
#define RAYGUI_LINE_TEXT_PADDING 4
  ...
  Color color = Fade(GetColor(GuiGetStyle(DEFAULT, (state == STATE_DISABLED)
                                                       ? BORDER_COLOR_DISABLED
                                                       : LINE_COLOR)),
                     guiAlpha);
...
#define RAYGUI_TABBAR_ITEM_WIDTH 160
  ...
  for (int i = 0; i < count; i++) {
    ...
    if (tabBounds.x < GetScreenWidth()) {
      ...
      if (i == *active)
        GuiToggle(tabBounds, GuiIconText(12, text[i]), true);
      else if (GuiToggle(tabBounds, GuiIconText(12, text[i]), false) == true)
        *active = i;
      GuiSetStyle(TOGGLE, TEXT_PADDING, textPadding);
      GuiSetStyle(TOGGLE, TEXT_ALIGNMENT, textAlignment);

      // Draw tab close button
      // NOTE: Only draw close button for curren tab: if
      // (CheckCollisionPointRec(mousePoint, tabBounds))
      int tempBorderWidth = GuiGetStyle(BUTTON, BORDER_WIDTH);
      int tempTextAlignment = GuiGetStyle(BUTTON, TEXT_ALIGNMENT);
      GuiSetStyle(BUTTON, BORDER_WIDTH, 1);
      GuiSetStyle(BUTTON, TEXT_ALIGNMENT, TEXT_ALIGN_CENTER);
...
Rectangle GuiScrollPanel(Rectangle bounds, const char *text, Rectangle content,
                         Vector2 *scroll) {
  GuiState state = guiState;
  ...
  GuiDrawRectangle(bounds, 0, BLANK,
                   Fade(GetColor(GuiGetStyle(DEFAULT, BACKGROUND_COLOR)),
                        guiAlpha)); // Draw background
...
bool GuiToggle(Rectangle bounds, const char *text, bool active) {
  ...
  if (state == STATE_NORMAL) {
    GuiDrawRectangle(
        ...
        Fade(GetColor(GuiGetStyle(TOGGLE, (active ? BORDER_COLOR_PRESSED
                                                  : (BORDER + state * 3)))),
             guiAlpha),
        Fade(GetColor(GuiGetStyle(
...
int GuiComboBox(Rectangle bounds, const char *text, int active) {
  ...
  GuiDrawText(
      items[active], GetTextBounds(COMBOBOX, bounds),
...
bool GuiDropdownBox(Rectangle bounds, const char *text, int *active,
                    bool editMode) {
  GuiState state = guiState;
  ...
  GuiDrawText(
      items[itemSelected], GetTextBounds(DEFAULT, bounds),
    ...
    for (int i = 0; i < itemCount; i++) {
      ...
      if (i == itemSelected) {
        GuiDrawRectangle(
            ...
            Fade(GetColor(GuiGetStyle(DROPDOWNBOX, BORDER_COLOR_PRESSED)),
                 guiAlpha),
            Fade(GetColor(GuiGetStyle(DROPDOWNBOX, BASE_COLOR_PRESSED)),
                 guiAlpha));
        GuiDrawText(items[i], GetTextBounds(DEFAULT, itemBounds),
                    GuiGetStyle(DROPDOWNBOX, TEXT_ALIGNMENT),
                    Fade(GetColor(GuiGetStyle(DROPDOWNBOX, TEXT_COLOR_PRESSED)),
                         guiAlpha));
      } else if (i == itemFocused) {
        GuiDrawRectangle(
            itemBounds, GuiGetStyle(DROPDOWNBOX, BORDER_WIDTH),
            Fade(GetColor(GuiGetStyle(DROPDOWNBOX, BORDER_COLOR_FOCUSED)),
                 guiAlpha),
            Fade(GetColor(GuiGetStyle(DROPDOWNBOX, BASE_COLOR_FOCUSED)),
                 guiAlpha));
        GuiDrawText(items[i], GetTextBounds(DEFAULT, itemBounds),
                    GuiGetStyle(DROPDOWNBOX, TEXT_ALIGNMENT),
                    Fade(GetColor(GuiGetStyle(DROPDOWNBOX, TEXT_COLOR_FOCUSED)),
                         guiAlpha));
      } else
        GuiDrawText(items[i], GetTextBounds(DEFAULT, itemBounds),
                    GuiGetStyle(DROPDOWNBOX, TEXT_ALIGNMENT),
                    Fade(GetColor(GuiGetStyle(DROPDOWNBOX, TEXT_COLOR_NORMAL)),
                         guiAlpha));
    }
  }
...
#define AUTO_CURSOR_DELAY 1     // Frames delay for autocursor movement
  ...
  case 0:
    ...
    break; // CENTERED
  case 1:
    cursor.y = textBounds.y - GuiGetStyle(DEFAULT, TEXT_SIZE) / 2;
    break; // UP
  case 2:
    cursor.y = textBounds.y + textBounds.height;
    break; // DOWN
  default:
    break;
  }
  ...
  if ((state != STATE_DISABLED) && !guiLocked && !guiSliderDragging) {
    ...
    if (editMode) {
      ...
      if ((textLength > textBoxCursorIndex) &&
          (IsKeyPressed(KEY_DELETE) ||
           (IsKeyDown(KEY_DELETE) &&
            (autoCursorCooldownCounter >= AUTO_CURSOR_COOLDOWN)))) {
        autoCursorDelayCounter++;

        if (IsKeyPressed(KEY_DELETE) ||
            (autoCursorDelayCounter % AUTO_CURSOR_DELAY) ==
      ...
      // cursor = GetCursorFromMousePosition(bounds, text, mouse); // Gui style
      // considered internally, including wrapMode

      // TODO: Get cursor rectangle from buffer index
      // cursor = GetCursorFromIndex(bounds, text, index);         // Gui style
      // considered internally, including wrapMode
      ...
      // Finish text editing on ENTER (if not multiline mode) or mouse click
      // outside bounds
      if ((!multiline && IsKeyPressed(KEY_ENTER)) ||
...
// unfortunately cursor placement is not working properly so the function is
removed bool GuiTextBoxMulti(Rectangle bounds, char *text, int bufferSize, bool
editMode)
{
    bool pressed = false;
...
bool GuiSpinner(Rectangle bounds, const char *text, int *value, int minValue,
  ...
  //--------------------------------------------------------------------
  pressed =
      GuiValueBox(spinner, NULL, &tempValue, minValue, maxValue, editMode);
...
bool GuiValueBox(Rectangle bounds, const char *text, int *value, int minValue,
                 int maxValue, bool editMode) {
...
float GuiSliderPro(Rectangle bounds, const char *textLeft,
  ...
  GuiDrawRectangle(
      ...
      Fade(GetColor(GuiGetStyle(SLIDER, (state != STATE_DISABLED)
                                            ? BASE_COLOR_NORMAL
                                            : BASE_COLOR_DISABLED)),
           guiAlpha));
...
float GuiSliderBar(Rectangle bounds, const char *textLeft,
                   const char *textRight, float value, float minValue,
                   float maxValue) {
  return GuiSliderPro(bounds, textLeft, textRight, value, minValue, maxValue,
                      0);
...
float GuiProgressBar(Rectangle bounds, const char *textLeft,
                     const char *textRight, float value, float minValue,
                     float maxValue) {
  GuiState state = guiState;
...
void GuiStatusBar(Rectangle bounds, const char *text) {
  ...
  GuiDrawRectangle(
      ...
      Fade(GetColor(GuiGetStyle(STATUSBAR, (state != STATE_DISABLED)
                                               ? BORDER_COLOR_NORMAL
                                               : BORDER_COLOR_DISABLED)),
           guiAlpha),
      Fade(GetColor(GuiGetStyle(STATUSBAR, (state != STATE_DISABLED)
...
void GuiDummyRec(Rectangle bounds, const char *text) {
  ...
  GuiDrawText(text, GetTextBounds(DEFAULT, bounds), TEXT_ALIGN_CENTER,
              Fade(GetColor(GuiGetStyle(BUTTON, (state != STATE_DISABLED)
                                                    ? TEXT_COLOR_NORMAL
                                                    : TEXT_COLOR_DISABLED)),
                   guiAlpha));
  //------------------------------------------------------------------
...
int GuiListViewEx(Rectangle bounds, const char **text, int count, int *focus,
  ...
  if (state != STATE_DISABLED) {
    ...
    for (int x = 0; x < checksX; x++) {
      for (int y = 0; y < checksY; y++) {
        ...
        GuiDrawRectangle(
            check, 0, BLANK,
            ((x + y) % 2) ? Fade(Fade(GetColor(GuiGetStyle(
                                          COLORPICKER, BORDER_COLOR_DISABLED)),
                                      0.4f),
                                 guiAlpha)
                          : Fade(Fade(GetColor(GuiGetStyle(
                                          COLORPICKER, BASE_COLOR_DISABLED)),
                                      0.4f),
                                 guiAlpha));
      }
    }
...
#define RAYGUI_TEXTINPUTBOX_HEIGHT 26
  ...
  if (secretViewActive != NULL) {
    ...
    *secretViewActive = GuiToggle(
        RAYGUI_CLITERAL(Rectangle){textBoxBounds.x + textBoxBounds.width -
                                       RAYGUI_TEXTINPUTBOX_HEIGHT,
...
Vector2 GuiGrid(Rectangle bounds, const char *text, float spacing,
                int subdivs) {
    ...
    if (subdivs > 0) {
      ...
      for (int i = 0; i < linesV; i++) {
        ...
        GuiDrawRectangle(lineV, 0, BLANK,
                         ((i % subdivs) == 0)
                             ? Fade(GetColor(GuiGetStyle(DEFAULT, LINE_COLOR)),
                                    RAYGUI_GRID_ALPHA * 4)
                             : Fade(GetColor(GuiGetStyle(DEFAULT, LINE_COLOR)),
                                    RAYGUI_GRID_ALPHA));
      }
    ...
    if (rgsFile != NULL) {
      ...
      if (fileDataSize > 0) {
        unsigned char *fileData =
            (unsigned char *)RL_MALLOC(fileDataSize * sizeof(unsigned char));
...
void GuiLoadStyleDefault(void) {
  ...
  GuiSetStyle(DEFAULT, TEXT_ALIGNMENT,
              TEXT_ALIGN_CENTER); // WARNING: Some controls use other values
  ...
  GuiSetStyle(TEXTBOX, TEXT_LINES_SPACING,
              (int)((float)GuiGetStyle(DEFAULT, TEXT_SIZE) * 1.5f));
...
#if !defined(RAYGUI_STANDALONE)
        DrawRectangle(posX + (k % RAYGUI_ICON_SIZE) * pixelSize,
                      posY + y * pixelSize, pixelSize, pixelSize, color);
#endif
      }
...
#define ICON_TEXT_PADDING 4
  ...
  if ((text != NULL) && (text[0] != '\0')) {
    ...
    // NOTE: We can't use GuiTextSplit() because it can be already used before
    // calling GuiDrawText() and static buffer would be overriden :(
    int lineCount = 0;
    ...
    for (int i = 0; i < lineCount; i++) {
      ...
      // WARNING: GetTextWidth() also processes text icon to get width! ->
      // Really needed?
      ...
      switch (alignmentVertical) {
      case 0:
        position.y = bounds.y + posOffsetY + bounds.height / 2 -
                     totalHeight / 2 + TEXT_VALIGN_PIXEL_OFFSET(bounds.height);
        break; // CENTERED
      case 1:
        position.y = bounds.y + posOffsetY;
        break; // UP
      case 2:
        position.y = bounds.y + posOffsetY + bounds.height - totalHeight +
      ...
      if (iconId >= 0) {
        ...
        GuiDrawIcon(iconId, (int)position.x,
...
#endif
      // DrawTextEx(guiFont, text, position, (float)GuiGetStyle(DEFAULT,
      // TEXT_SIZE), (float)GuiGetStyle(DEFAULT, TEXT_SPACING), tint);
      ...
      for (int c = 0, codepointSize = 0; c < size; c += codepointSize) {
        ...
        if (codepoint == '\n')
          break; // WARNING: Lines are already processed manually, no need to
                 // keep drawing after this codepoint
        else {
          if ((codepoint != ' ') && (codepoint != '\t')) {
            ...
            if (textOffsetX < (bounds.width - guiFont.recs[index].width)) {
              DrawTextCodepoint(
                  guiFont, codepoint,
                  RAYGUI_CLITERAL(Vector2){position.x + textOffsetX,
                                           position.y + textOffsetY},
                  (float)GuiGetStyle(DEFAULT, TEXT_SIZE), tint);
            }
          }
      ...
      // TODO: Allow users to set line spacing for text: GuiSetStyle(TEXTBOX,
      // TEXT_LINES_SPACING, x)
...
static void GuiDrawRectangle(Rectangle rec, int borderWidth, Color borderColor,
  ...
  if (borderWidth > 0) {
    ...
    DrawRectangle((int)rec.x, (int)rec.y + (int)rec.height - borderWidth,
                  (int)rec.width, borderWidth, borderColor);
  }
}
...
static int GuiScrollBar(Rectangle bounds, int value, int minValue,
  ...
  if ((state != STATE_DISABLED) && !guiLocked) {
    ...
    if (guiSliderDragging) // Keep dragging outside of bounds
    {
      if (IsMouseButtonDown(MOUSE_LEFT_BUTTON)) {
        if (CHECK_BOUNDS_ID(bounds, guiSliderActive)) {
          if (isVertical)
            value += (GetMouseDelta().y / (scrollbar.height - slider.height) *
                      valueRange);
          else
            value += (GetMouseDelta().x / (scrollbar.width - slider.width) *
                      valueRange);
        }
      } else {
        guiSliderDragging = false;
  ...
  GuiDrawRectangle(scrollbar, 0, BLANK,
  ...
  if (GuiGetStyle(SCROLLBAR, ARROWS_VISIBLE)) {
    ...
    GuiDrawText(
        ...
        Fade(GetColor(GuiGetStyle(SCROLLBAR, TEXT + state * 3)),
             guiAlpha)); // ICON_ARROW_UP_FILL / ICON_ARROW_LEFT_FILL
    GuiDrawText(
        isVertical ? "#120#" : "#119#",
        RAYGUI_CLITERAL(Rectangle){arrowDownRight.x, arrowDownRight.y,
                                   isVertical ? bounds.width : bounds.height,
        ...
        TEXT_ALIGN_CENTER,
        Fade(GetColor(GuiGetStyle(SCROLLBAR, TEXT + state * 3)),
             guiAlpha)); // ICON_ARROW_DOWN_FILL / ICON_ARROW_RIGHT_FILL
#endif
  }
  //--------------------------------------------------------------------
